...
-- Enables automatic, atomic, server-derived exactly-once inventory consumption when orders reach 'served' status.

-- 1. Add inventory consumption columns to session_orders
ALTER TABLE public.session_orders
  ADD COLUMN IF NOT EXISTS inventory_consumed_at TIMESTAMP WITH TIME ZONE,
  ADD COLUMN IF NOT EXISTS inventory_consumption_status TEXT DEFAULT 'pending',
  ADD COLUMN IF NOT EXISTS inventory_consumption_notes TEXT;

-- 2. Add order_id reference to stock_movements
ALTER TABLE public.stock_movements
  ADD COLUMN IF NOT EXISTS order_id UUID REFERENCES public.session_orders(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_stock_movements_order ON public.stock_movements(order_id);

-- 3. Stored Procedure / RPC for Atomic, Exactly-Once Inventory Consumption
...
BEGIN
  ...
  -- 2. Process each item in session_orders.items
  ...
  -- 3. Case: No ingredients required (all items missing recipes or empty)
  ...
  -- 4. Check stock availability (lock inventory rows in sorted ID order to prevent deadlocks)
  FOR v_inv IN 
  ...
  -- 5. Atomic Stock Deduction & Immutable Stock Movement Creation
  FOR v_inv IN 
  ...
  -- 6. Mark session order as successfully consumed
  UPDATE public.session_orders
  SET 
    inventory_consumed_at = NOW(),
    inventory_consumption_status = CASE 
